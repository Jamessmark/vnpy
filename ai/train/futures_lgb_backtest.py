...
class FuturesLongShortStrategy(AlphaStrategy):
    ...
    min_hold_days: int = 3    # 最小持仓天数（未达到则不允许换出）
...
def main() -> None:
    ...
    # - infer 不处理（保留所有样本用于推理）
    # - learn 只对 label 去 NaN 再做截面 zscore 归一化
    ...
    # ── Step 4~7  三模型全流程：训练 → IC → 回测 → 汇总对比 ───────────────
    print("\n" + "=" * 60)
    print("Step 4~7  三模型全流程对比（LGB / Lasso / MLP）")
    print("=" * 60)

    import numpy as np
    from scipy.stats import spearmanr

    N_FEATURES = 158  # Alpha158 因子数，MLP input_size

    model_configs = [
        ("LGB",   LgbModel(**LGB_PARAMS)),
        ("Lasso", LassoModel(alpha=0.0005, random_state=42)),
        ("MLP",   MlpModel(
    ...
    for model_name, model in model_configs:
        ...
        try:
            ...
            print(f"  ❌ 训练失败：{e}")
            summary.append({"model": model_name, "ic_mean": float("nan"),
                             "sharpe": float("nan"), "annual_return": float("nan"),
                             "max_ddpercent": float("nan"), "trades": 0, "commission": 0})
            continue
        ...
        if daily_df is not None:
            ...
            ann_ret = sharpe = max_ddpct = rdr = float("nan")
            n_trades = commission = 0
            print("  ❌ 回测无成交")
        ...
        # 可选：保存最后一个模型的信号（或按需修改）
        if SAVE_ARTIFACTS:
            lab.save_model(f"{ARTIFACT_NAME}_{model_name.lower()}", model)
            lab.save_signal(f"{ARTIFACT_NAME}_{model_name.lower()}", sig_df)

    # ── 汇总对比表 ──────────────────────────────────────────────────────────
    print("\n\n" + "=" * 70)
    print("  最终汇总对比")
    print("=" * 70)
    print(f"  {'模型':<8}  {'IC Mean':>8}  {'ICIR':>7}  {'年化收益%':>9}  {'Sharpe':>7}  {'最大回撤%':>9}  {'笔数':>6}")
    print(f"  {'-'*8}  {'-'*8}  {'-'*7}  {'-'*9}  {'-'*7}  {'-'*9}  {'-'*6}")
    for row in summary:
        ic_m  = f"{row['ic_mean']:+.4f}"   if not (row['ic_mean'] != row['ic_mean']) else "  N/A  "
        icir_ = f"{row['icir']:+.4f}"       if not (row['icir']    != row['icir'])    else "  N/A  "
        ann_  = f"{row['annual_return']:.2f}" if not (row['annual_return'] != row['annual_return']) else " N/A"
        shp_  = f"{row['sharpe']:.3f}"      if not (row['sharpe']  != row['sharpe'])  else " N/A"
        dd_   = f"{row['max_ddpercent']:.2f}" if not (row['max_ddpercent'] != row['max_ddpercent']) else " N/A"
        print(f"  {row['model']:<8}  {ic_m:>8}  {icir_:>7}  {ann_:>9}  {shp_:>7}  {dd_:>9}  {row['trades']:>6}")

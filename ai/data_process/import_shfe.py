"""
SHFE（上海期货交易所）历史日K线数据导入工具

将 ai/data/shfe/ 目录下的 Excel 文件导入到 vnpy 数据库
...
使用方法：
    # 先测试导入3条
    uv run python ai/data_process/import_shfe.py --test --limit 3

    # 测试全部导入（前100条）
    uv run python ai/data_process/import_shfe.py --test --limit 100

    # 全部导入（覆盖已有数据）
    uv run python ai/data_process/import_shfe.py --force

    # 导入指定年份
    uv run python ai/data_process/import_shfe.py --year 2026 --force
"""
...
def get_all_excel_files(data_dir: Path, year: int = None) -> list:
    ...
    # 排除嵌套的 INE 目录
    files = [f for f in files if "ine" not in str(f).lower()]
...
def df_to_bars(df: pd.DataFrame) -> list:
    ...
    for _, row in df.iterrows():
        try:
            ...
            turnover = pd.to_numeric(row.get('成交金额(万元)', 0), errors='coerce')
...
def import_shfe_data(
    ...
    """导入 SHFE 数据"""
    data_dir = Path(__file__).parent.parent / "data" / "shfe"
    ...
    for excel_file in files:
        print(f"📖 读取 {excel_file.parent.name}/{excel_file.name}...")
    ...
    try:
        if force:
            ...
            for symbol, symbol_bars in sorted(bars_by_symbol.items()):
                ...
                bars_to_save = list(existing)  # 先复制已有数据
                
                for bar in symbol_bars:
                    if bar.datetime.date() not in existing_dates:
                        bars_to_save.append(bar)
                        existing_dates.add(bar.datetime.date())  # 防止新数据中有重复
...
def main():
    parser = argparse.ArgumentParser(description="SHFE 历史数据导入工具")

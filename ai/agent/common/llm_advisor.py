...
给出综合评分（-100到100）：
- 正值表示看多，负值表示看空
- **必须用加粗数字格式输出，例如：** `**+65**` 或 `**-40**`（不要加 /100、分等后缀）
- 紧跟评分后用括号说明依据，例如：**+65**（主要理由）
...
# 多品种（产业链分组）Prompt 模板
_GROUP_PROMPT_TEMPLATE = """你是一位专业的商品期货分析师。以下是同一产业链的多个品种数据，请结合产业链逻辑和品种间联动关系，逐一生成每个品种的交易决策建议。
...
## 任务要求
针对上述每个品种，**逐一**输出如下格式（每个品种单独一个完整分析块）：

---
### 品种：{variety_names_list}中的每一个

请对每个品种输出以下结构（用 ## 品种名 开头区分）：

## [品种名]

### 市场分析
（2-3句话，结合产业链背景和新闻关键信息）
...
### 综合评分
给出综合评分（-100到100）：正值看多，负值看空，说明依据。
**必须用加粗数字格式输出，例如：** `**+65**` 或 `**-40**`（不要加 /100、分等后缀），紧跟括号说明理由，例如：**+65**（主要理由）。
